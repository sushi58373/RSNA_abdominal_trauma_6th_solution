...
# Config
kernel_type = 'extra-feat-sliding-seq'
load_kernel = None
load_last = True
...
emb_dir = os.path.join(data_dir, f"feature_extracted/sliding_{in_chans}_no_seg") # embedding directory
...
class SeqModel(nn.Module):
    def __init__(self):
        ...
        self.lstm1 = nn.LSTM(lstm_in_dim*3, lstm_size, bidirectional=True, batch_first=True)
...
def train_func(model, loader_train, optimizer, scaler=None):
    ...
    for embs, mask, study_label, image_labels in bar:
        ...
        with amp.autocast():
            ...
            # study_loss = criterion(logits, study_label)
            # image_loss = criterion(image_logits, image_labels)
            loss = study_loss * 0.9 + image_loss * 0.1
...
def run(df, fold):
    ...
    dataset_valid = CLSExtraSeqDataset(valid_, valid_psids, 'train')
...
def evaluation(df, model_folder):
    ...
    valid_psids_lst = []
    for fold in range(5):
        model_file = os.path.join(model_folder, f'{kernel_type}_fold{fold}_best.pth')
    ...
    pd.DataFrame({'psid':valid_psids_lst, 'extra_output':study_outputs}
        ).to_csv(os.path.join(model_folder, 'extra_output.csv'), index=False)
