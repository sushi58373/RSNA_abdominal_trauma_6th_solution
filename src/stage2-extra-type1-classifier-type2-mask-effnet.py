...
# Config
kernel_type = 'extra-feat-sliding-seg-eff'
load_kernel = None
load_last = True
...
image_size = 384 # to (384), (512)
in_chans = 5

init_lr = 0.0001 # 23e-5
eta_min = 0 # 23e-6
batch_size = 64
drop_rate = 0.
drop_rate_last = 0.3
...
def run(df, df2, fold):
    ...
    model = TimmEffFeatExtractorSeg(backbone, decoder_channels, pretrained=True)
...
def save_npy(df, fold):
    out_path = os.path.join(data_dir, f'feature_extracted/sliding_{in_chans}_eff')
    ...
    model = TimmEffFeatExtractorSeg(backbone, decoder_channels, pretrained=False)
    model_file = f"../results/stage2-extra-type1-seg/extra-feat-sliding-seg-eff/model/extra-feat-sliding-seg-eff_fold{fold}_best.pth"
    ...
    with torch.no_grad():
        for i, (images, mask, study_labels, image_labels) in bar:
            ...
            feat = model.hidden_layer(feats[-1]).detach().cpu().numpy()
...
def evaluation(df, model_folder):
    ...
    for fold in range(5):
        ...
        model = TimmEffFeatExtractorSeg(backbone, decoder_channels, pretrained=False)
        path = os.path.join(model_folder, f'extra-feat-sliding-seg_fold{fold}_best.pth')
    ...
    valid.to_csv(os.path.join(model_folder, 'df.csv'), index=False)
...
if __name__ == "__main__":
    ...
    if 0:
        m = TimmEffFeatExtractorSeg(backbone, decoder_channels)
        study_logit, image_logit, seg = m(torch.rand(2, in_chans, image_size, image_size))
        print(study_logit.shape, image_logit.shape, seg.shape)
